...
describe Dog do
  ...
  subject(:best_pet) { described_class.new('spot','golden retreiver', 'gold') } 

  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'does something useful when it is talked to' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(best_pet).not_to be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(best_pet).to be_sleeping
  end
end
