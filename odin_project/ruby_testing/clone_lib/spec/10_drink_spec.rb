...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      let(:testade) { described_class.new('testade') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(testade.type).to eq('testade')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(testade.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:clock_water) { described_class.new('clock water') }

      # remove the 'x' before running this test
      it 'is full' do
        expect(clock_water).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:duck_drink) { described_class.new('duck drink', 9) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(duck_drink).not_to be_full
      end
    end
  end
end
